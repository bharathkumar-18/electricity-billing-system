...
<link rel="stylesheet" href="paymentStatus_Styles2.css">
...
<body>
	...
	<%
		...
		if(areCardDetailsCorrect) {
			msg = "Your payment is successful. Thankyou";	
	%>
	<div class='status-box'>
		<h1><%=msg %></h1>
		<a class="links" href="<%=request.getContextPath()%>/bill.jsp"><button type="button" value="homepage"> Go To Homepage</button></a>
	</div>
	<%
		}
		else{
			msg="Your payment failed. Kindly retry your payment";
			
	%>
			<div class='status-box'>
			<h1><%=msg %></h1>
			<a class="links" href="<%=request.getContextPath()%>/makePayment.jsp"><button type="button" value="homepage"> Retry payment</button></a>
		</div>
	<%
		}
	%>
